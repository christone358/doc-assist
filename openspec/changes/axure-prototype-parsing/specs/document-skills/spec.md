...
### Requirement: 文档编写 Skill 可按页面使用原型事实
需要描述真实界面内容的文档编写 Skill SHALL 能够按模块关联页面读取原型事实，并基于结构化页面要素进行写作，而不是直接依赖原始 HTML 源码或凭空推断界面内容。

#### Scenario: Skill 先定位模块关联页面再读取详情
- **WHEN** 文档编写 Skill 需要描述某个模块的页面操作步骤或界面结构
- **THEN** Skill SHALL 先读取该模块关联页面列表，再按需读取具体页面的结构化页面事实
